...
function grupos_creados = CrearGrupos(puntos, cantidad,umbral)
    ...
    for i = 1:1:cantidad
            ...
            if i == 1
                ...
                [grp_pert,distancia] = Det_Distancias(puntos(1,i),puntos(2,i),grupos,j);
                if distancia < umbral
                    ...
                    if validar_vacio == 0
                        grupos{grp_pert,longitud_total(2)+1} =[puntos(1,i),puntos(2,i)];
                        %fprintf("Longitud del grupo %d es: %d\n",grp_pert,longitud_total(2)+1);
                    else
                        grupos{grp_pert,validar_vacio} =[puntos(1,i),puntos(2,i)];
                        %fprintf("Longitud del grupo %d es: %d\n",grp_pert,validar_vacio);
                    end
                else
                    j = j+1;
                    grupos{j,1} = [puntos(1,i),puntos(2,i)];
